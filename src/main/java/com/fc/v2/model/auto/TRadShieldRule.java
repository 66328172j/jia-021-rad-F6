...
/**
 * 屏蔽体核算判定规则对象 t_rad_shield_rule
 *
 * @author fuce
 * @date 2026-09-12
 */
@TableName("t_rad_shield_rule")
@ApiModel(value = "TRadShieldRule", description = "屏蔽体核算判定规则")
public class TRadShieldRule implements Serializable {
    ...
    /** 屏蔽余量不足档上限(%) */
    @TableField("th1_max")
    @ApiModelProperty(value = "屏蔽余量不足档上限(%)")
    private BigDecimal th1Max;

    /** 屏蔽余量临界档上限(%) */
    @TableField("th2_max")
    @ApiModelProperty(value = "屏蔽余量临界档上限(%)")
    private BigDecimal th2Max;

    /** 屏蔽余量达标档上限(%) */
    @TableField("th3_max")
    @ApiModelProperty(value = "屏蔽余量达标档上限(%)")
