...
/**
 * 分区驻留剂量判定规则对象 t_rad_zone_rule
 *
 * @author fuce
 * @date 2026-09-12
 */
@TableName("t_rad_zone_rule")
@ApiModel(value = "TRadZoneRule", description = "分区驻留剂量判定规则")
public class TRadZoneRule implements Serializable {
    ...
    /** 驻留剂量临时档上限(μSv) */
    @TableField("th1_max")
    @ApiModelProperty(value = "驻留剂量临时档上限(μSv)")
    private BigDecimal th1Max;

    /** 驻留剂量短时档上限(μSv) */
    @TableField("th2_max")
    @ApiModelProperty(value = "驻留剂量短时档上限(μSv)")
    private BigDecimal th2Max;

    /** 驻留剂量限时档上限(μSv) */
    @TableField("th3_max")
    @ApiModelProperty(value = "驻留剂量限时档上限(μSv)")
