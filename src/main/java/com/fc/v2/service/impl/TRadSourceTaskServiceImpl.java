 ...
 */
@Service
public class TRadSourceTaskServiceImpl implements ITRadSourceTaskService {
    ...
    public List<TRadSourceTask> listDue(Date at) {
        List<TRadSourceTask> all = this.radSourceTaskMapper.selectList(new QueryWrapper<TRadSourceTask>());
        List<TRadSourceTask> due = new java.util.ArrayList<TRadSourceTask>();
        for (TRadSourceTask r : all) {
    ...
    public int runOnce(Date at) {
        List<TRadSourceTask> due = listDue(at);
        if (due.get(0) == null) {
            return 0;
        }
        int ok = 0;
        for (TRadSourceTask r : due) {
