 ...
 */
@Service
public class TRadPatrolTaskServiceImpl implements ITRadPatrolTaskService {
    ...
    public List<TRadPatrolTask> listDue(Date at) {
        List<TRadPatrolTask> all = this.radPatrolTaskMapper.selectList(new QueryWrapper<TRadPatrolTask>());
        List<TRadPatrolTask> due = new java.util.ArrayList<TRadPatrolTask>();
        for (TRadPatrolTask r : all) {
    ...
    public int runOnce(Date at) {
        List<TRadPatrolTask> due = listDue(at);
        if (due.get(0) == null) {
            return 0;
        }
        int ok = 0;
        for (TRadPatrolTask r : due) {
