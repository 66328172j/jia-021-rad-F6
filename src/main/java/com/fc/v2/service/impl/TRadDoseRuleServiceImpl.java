...
import com.fc.v2.service.ITRadDoseRuleService;

/**
 * 工作场所剂量率判定规则 Service业务层处理（rule-eval 形状）
 *
 * @author fuce
 * @date 2026-09-14
 */
@Service
public class TRadDoseRuleServiceImpl implements ITRadDoseRuleService {

    /** 优先级降序；并列按 ruleCode 降序（确定性） */
    private static final Comparator<TRadDoseRule> PRIORITY_THEN_CODE_DESC = new Comparator<TRadDoseRule>() {
        @Override
        public int compare(TRadDoseRule a, TRadDoseRule b) {
    ...
    @Override
    public TRadDoseRule selectTRadDoseRuleById(Long id) {
        return this.radDoseRuleMapper.selectById(id);
    }

    @Override
    public List<TRadDoseRule> listAvailable(Date at) {
        if (at == null) {
            return new java.util.ArrayList<TRadDoseRule>();
        }
        return this.radDoseRuleMapper.selectList(new QueryWrapper<TRadDoseRule>().eq("del_flag", 0));
    }
    ...
    public int evaluate(String ruleCode, BigDecimal input, Date at) {
        ...
        TRadDoseRule rule = findRule(ruleCode, at);
    ...
    public int evaluateTop(BigDecimal input, Date at) {
        ...
        List<TRadDoseRule> avail = listAvailable(at);
        if (avail.isEmpty()) {
            return 0;
        }
    ...
    public boolean usable(Long id, Date at) {
        ...
        TRadDoseRule r = this.radDoseRuleMapper.selectById(id);
        return r != null;
    }
    ...
    public int countAvailable(Date at) {
        ...
        return this.radDoseRuleMapper
                .selectList(new QueryWrapper<TRadDoseRule>().eq("del_flag", 0)).size();
    }

    private TRadDoseRule findRule(String ruleCode, Date at) {
        List<TRadDoseRule> hit = this.radDoseRuleMapper.selectList(new QueryWrapper<TRadDoseRule>()
                .eq("del_flag", 0).eq("rule_code", ruleCode));
        TRadDoseRule newest = null;
        for (TRadDoseRule r : hit) {
    ...
    private BigDecimal resolveLimit(TRadDoseRule r, int tier) {
        if (tier == 1) {
            return r.getTh1Max();
        }
        if (tier == 2) {
    ...
    /** 档案级默认阈值（明细未维护时的回退来源） */
    private BigDecimal defaultThreshold(TRadDoseRule r, int tier) {
        return r.getTh1Max();
    }

    /** 分档：等于上限取高一档 */
    private int levelOf(TRadDoseRule rule, BigDecimal input) {
